...
/*
 * Copyright [2009] [Marcin Rzeznicki]
 ...
 * First version written by Marcin Rzeznick was to work work with Apachi.2.2.
 * I have modified it to work with Apachi 3.2. Some of the methods related to
 * the matrices are deprecated in apachi 3.2.  Changes are only made in
 * computeSGCoefficients(int nl, int nr, int degree) method see comments below
 * for the changes made by Muhammad
 *
 * @author Marcin Rzeznicki
 * @author Muhammad Muaaz
 * @Version 1.0
 * @modifed 23-10-2014
 */
public class SGFilterModified {
     ...
     */
    public SGFilterModified(int nl, int nr) {
        if (nl < 0 || nr < 0)
     ...
     * @param degree order of smoothin polynomial
     * @return Savitzky-Golay coefficients
     * @throws IllegalArgumentException if {@code nl < 0} or {@code nr < 0} or {@code nl + nr <
     *                                  degree}
     */
     ...
     * @param index      where it should be placed in data filters queue
     * @see DataFilter
     */
     ...
     *
     * @param p     preprocessor
     * @param index where it should be placed in preprocessors queue
     * @see Preprocessor
     */
     ...
     * Removes data filter
     *
     * @param dataFilter data filter to be removed
     ...
     * Removes data filter
     *
     * @param index which data filter to remove
     ...
     *
     * @param index which preprocessor to remove
     ...
     * Removes preprocessor
     *
     * @param p preprocessor to be removed
     ...
     * @return filtered data
     * @throws IllegalArgumentException  when <tt>bias < 0</tt> or <tt>bias > min(nr, nl)</tt>
     * @throws IndexOutOfBoundsException when {@code coeffs} has less than <tt>2*bias + 1</tt>
     *                                   elements
     * @throws NullPointerException      when any array passed as parameter is null
     */
    public double[] smooth(double[] data, double[] leftPad, double[] rightPad,
     ...
     * @throws IllegalArgumentException       if <tt>from < 0</tt> or <tt>to > data.length</tt>
     * @throws NullPointerException           if {@code data} is null or {@code coeffs} is null
     */
     ...
     * @param coeffs filter coefficients
     * @return filtered data
     * @throws ArrayIndexOutOfBoundsException if <tt>to > data.length</tt> or when {@code coeffs} has less
     *                                        than <tt>2*bias + 1</tt> elements
     * @throws IllegalArgumentException       if <tt>from < 0</tt> or <tt>to > data.length</tt> or
