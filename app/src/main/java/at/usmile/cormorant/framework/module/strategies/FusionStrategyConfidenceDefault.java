...
public class FusionStrategyConfidenceDefault implements FusionStrategy {
    ...
    public double fuseData() {
        ...
        //TODO consider date and status (also for risk)
        for (PluginInfo eachApi : PluginManager.getInstance().getPluginListReadOnly()) {
            if(CormorantConstants.PLUGIN_TYPE.confidence.equals(eachApi.getPluginType())){
                sum += eachApi.getStatusDataConfidence().getConfidence();
                count++;
            }
        }
        return sum / count;
